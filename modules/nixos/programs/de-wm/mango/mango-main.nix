...
delib.module {
  ...
  home.ifEnabled =
    ...
    let
      baseWindowRules = [
        "isfloating:1,appid:^(mpv|imv|showmethekey-gtk)$"
        "isfloating:1,isnoanimation:1,isnoshadow:1,noblur:1,isopensilent:1,appid:^(ueberzugpp_layer)$"
        "isfloating:1,width:1200,height:800,appid:^(org.kde.gwenview)$"
        "isfloating:1,width:900,height:600,title:^(Open File|Select a File|Choose wallpaper|Open Folder|Save As|Library|File Upload|Save File|Enter name of file)(.*)$"
    ...
    with config.lib.stylix.colors;
    {
      ...
      wayland.windowManager.mango = {
        ...
        settings = {
          monitorrule = cfg.monitors;
          windowrule = baseWindowRules ++ cfg.windowRules;
