...
in
{
  config = lib.mkIf
    ((vars.niri or false)
      || ((vars.hyprland or false) && !(vars.caelestia or false)))
    {
        ...
        settings = {
          mainBar = {
            ...
            "battery" = {
              ...
              format-charging =
                "<span color='${c.base0B}'></span> {capacity}%";
      ...
      systemd.user.services.waybar = {
        Unit = {
          PartOf = lib.mkForce
            ((if (vars.niri or false) then [ "niri.service" ] else [ ])
              ++ (if ((vars.hyprland or false)
              && !(vars.caelestia or false)) then
        ...
        Install.WantedBy = lib.mkForce
          ((if (vars.niri or false) then [ "niri.service" ] else [ ])
            ++ (if ((vars.hyprland or false) && !(vars.caelestia or false)) then
            [ "hyprland-session.target" ]
          else
            [ ]));
      };
