...
**Rule of thumb:** before researching a package/option, verifying a build, debugging a failure, checking cross-arch compat, linting, or writing tests yourself, ask "is there an agent for this?" If yes, use it - don't do it in the main loop. Only work directly when no agent fits: quick one-line answers, reading a single file to answer a question, or actions trivial enough that delegating would cost more tokens than doing them.
