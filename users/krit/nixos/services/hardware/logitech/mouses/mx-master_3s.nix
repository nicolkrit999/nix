...
delib.module {
  ...
  nixos.ifEnabled = {
    ...
    environment.etc."logid-mx3s.cfg".text = ''
      devices: (
        {
          name: "MX Master 3S";
          productId: 0xb034;
    ...
    systemd.services.logid-mx3s = {
      description = "Logitech Configuration Daemon (MX Master 3S)";
      wantedBy = [ "multi-user.target" ];
      serviceConfig = {
        ExecStart = "${pkgs.logiops}/bin/logid -c /etc/logid-mx3s.cfg";
        Restart = "always";
        RestartSec = "3s";
      };
    };
