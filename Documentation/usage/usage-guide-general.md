...
Add the enable flag to `hosts//variables.nix` for *every* host (set to `false` by default, `true` where needed).
```nix
# variables.nix
cinnamon = false;
...
Use this when you need complex Home Manager modules that apply *only* to one specific machine.
...
* *Note:* `/home/.snapshots` is a **dedicated subvolume**, separate from `/home`.
