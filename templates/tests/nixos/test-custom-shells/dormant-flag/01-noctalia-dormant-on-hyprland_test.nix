# D01 — Noctalia dormant on Hyprland (enable=false, enableOnHyprland=true)
# Regression test for the 2026-04-21 swaync/ProtonVPN incident.
# Expected: must behave exactly like P03 (no active shell).
{ nix-tests }:
let
  H = import ../shared/eval-scenario.nix;
  lib = H.lib;
  config = H.getConfig ./01-noctalia-dormant-on-hyprland;
  hm = H.getHm config;
  binds = hm.wayland.windowManager.hyprland.settings.bind;
in
nix-tests.runTests {
  "D01: noctalia dormant on hyprland — must behave as no-shell" = helpers: {
